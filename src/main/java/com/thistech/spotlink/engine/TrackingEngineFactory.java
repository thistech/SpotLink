...
public class TrackingEngineFactory {
    private static final Logger log = LoggerFactory.getLogger(TrackingEngineFactory.class);

    public TrackingEngine newInstance(Properties properties) {
        String className = properties.getProperty("trackingEngine");
        try {
            @SuppressWarnings("unchecked")
            Class<TrackingEngine> clazz = (Class<TrackingEngine>) Class.forName(className);
